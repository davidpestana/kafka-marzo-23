...
public class SchemaRegistryConsumerTopicStrategy {
	...
	public static void main(String[] args) throws IOException {
		...
		props.put(ConsumerConfig.GROUP_ID_CONFIG, "SchemaRegistryConsumerRecordNameStrategy");
		props.put(ConsumerConfig.KEY_DESERIALIZER_CLASS_CONFIG, KafkaAvroDeserializer.class.getName());
		props.put(ConsumerConfig.VALUE_DESERIALIZER_CLASS_CONFIG, KafkaAvroDeserializer.class.getName());
		props.put(KafkaAvroDeserializerConfig.SCHEMA_REGISTRY_URL_CONFIG, "http://localhost:8081");
		
		KafkaConsumer<Ciudad, GenericRecord> consumer = new KafkaConsumer<>(props);
		consumer.subscribe(Collections.singletonList(SchemaRegistryProducer.TOPIC_BASE));
		while(!closed.get()) {
			ConsumerRecords<Ciudad, GenericRecord> records = consumer.poll(Duration.ofSeconds(1));
