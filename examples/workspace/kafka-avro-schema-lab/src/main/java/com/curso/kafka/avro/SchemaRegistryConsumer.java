...
public class SchemaRegistryConsumer {
	...
	public static void main(String[] args) throws IOException {
		...
		props.put(ConsumerConfig.GROUP_ID_CONFIG, "SchemaRegistryConsumer");
		props.put(ConsumerConfig.KEY_DESERIALIZER_CLASS_CONFIG, StringDeserializer.class.getName());
		props.put(ConsumerConfig.VALUE_DESERIALIZER_CLASS_CONFIG, KafkaAvroDeserializer.class.getName());
		props.put(KafkaAvroDeserializerConfig.SCHEMA_REGISTRY_URL_CONFIG, "http://localhost:8081");
		
		KafkaConsumer<String, Clima> consumer = new KafkaConsumer<>(props);
		consumer.subscribe(Collections.singletonList(SchemaRegistryProducer.TOPIC_BASE));
		while(!closed.get()) {
			ConsumerRecords<String, Clima> records = consumer.poll(Duration.ofSeconds(1));
